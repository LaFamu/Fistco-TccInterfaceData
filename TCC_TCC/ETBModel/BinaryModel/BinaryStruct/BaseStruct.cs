...
namespace TCC_TCC.A.ETBModel
{
    public abstract class BaseStruct
    {
        ...
        /// 
        /// </summary>
        /// <param name="blockid">接口码位表中的数据块中序号</param>
        /// <param name="packoffset">本数据块在数据包格式表中的偏移地址</param>
        /// <param name="blockWidth">本数据块所有信息位宽</param>
        /// <param name="blockWidthOffset">本数据块的本信息前面信息的位宽的和</param>
        /// <param name="infoWidth">“数据块格式表”中的本数据块的本信息的位宽</param>
        /// <returns></returns>
        protected CommAddress GetCommAddress(UInt32 blockid, UInt32 packoffset, UInt32 blockWidth, UInt32 blockWidthOffset,UInt32 infoWidth)
        {
            CommAddress result = new CommAddress();

            result.ByteOffset = packoffset + ((blockid - 1) * blockWidth + blockWidthOffset) / 8;
            result.BitOffset = ((blockid - 1) * blockWidth + blockWidthOffset) % 8;
            result.Width = infoWidth;
        ...
        protected DEVICE_ID GetBitMapDEVICE_ID<T>(UInt32 deviceid, List<T> list) where T : BitMapBase
        {
            DEVICE_ID device_ID = new DEVICE_ID();
            if (deviceid != UInt32.MaxValue)
            {
                device_ID.Id = list.FirstOrDefault(x => x.datablockId == deviceid).datablockId << 16 ^ list.FirstOrDefault(x => x.datablockId == deviceid).datablockId;
                device_ID.Index = (UInt32)list.Select(x => x.datablockId).ToList().IndexOf(deviceid);
            }
        ...
        //        {
        //            if(AllDataModel.InterLockData.InterfaceDefineList.FirstOrDefault(x => x.Name == autoClose.InterfaceName).InterfaceType == "CBI-TCC_TCC")
        //            {
