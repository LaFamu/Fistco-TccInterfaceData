...
namespace JSDG.Platform.Content
{
    ...
    /// </summary>
    public partial class TCC_TCCDataGeneratorUC : System.Windows.Controls.UserControl
    {
        ...
        public void Debug()
        {
            this.InputPathTextBox.Text = @"C:\Users\gongzhiqiao\Desktop\TCC数据工具\Tcc接口数据表_同济站";
            this.OutputPathTextBox.Text = @"C:\Users\gongzhiqiao\Desktop\TCC数据工具\Tcc接口数据表_同济站";
        }
        ...
        private void bgw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                String message = String.Format("T2T通讯数据转换失败，发生以下错误：\r\n{0}", e.Error.Message);
            ...
            {
                ModernDialog.ShowMessage("T2T通讯数据转换失败！              ", "温馨提示", MessageBoxButton.OK);
            }
            else
            {
                ...
                ModernDialog.ShowMessage("T2T通讯数据转换成功！              ", "温馨提示", MessageBoxButton.OK);
        ...
        private void bgw_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            _outputDirectory = string.Format("TccAppInf_{0}", DateTime.Now.ToString("yyyyMMdd"));
            if (_index == 0)
            {
                if (App.Container.Act_TCC_TCC_A == null) throw new Exception("缺少DLL文件");
                fileNamesA = FileNamesHelper.GetFileNames(_inputPath, ESubSystemType.TccBit);
                _outputPathA = String.Format("{0}\\{1}\\A\\", _outputPath, _outputDirectory);
                LogHelper.Instance.Init(ESubSystemType.TccBit, _outputPathA);
             
                WriteToAlarmAndLog(string.Format("{0}: 数据生成开始：", DateTime.Now.ToString()), Brushes.Gray);
                App.Container.Act_TCC_TCC_A(_outputPathA, fileNamesA, _bgw.ReportProgress);
            }
            else
            {
