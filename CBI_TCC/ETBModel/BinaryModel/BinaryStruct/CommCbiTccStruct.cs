...
namespace JSDG.CBI.A.ETBModel
{
    public class CommCbiTccStruct
    {
        ...
        {
            MsgHelper.Instance(1, string.Format("正在生成结构体《CommCbiTccStruct》"));
            sourceList = dataModel.InterLockData.CommCbiTccList;
            Parameter.TotalStruct = 1;
            Parameter.StructWidth = 31948;
            Parameter.SegmentName = "CommCbiTccStruct";
        ...
        private void Transform()
        {
            ...
            try
            {
                ...
                {
                    if (sourceList[i].Direction.Contains("CBI-TCC"))
                    {
                    ...
                    if (sourceList[i].Direction.Contains("TCC-CBI"))
                    {
                        Cnt_R = 0;
